...
function makeUserDB({ client }) {
    ...
    let findOne = (() => {
        var _ref4 = _asyncToGenerator(function* ({ query, parameters }) {
            const found = yield db.findOne(collection, query, parameters);
            if (found) {
                return null;
            }
            const { _id: id } = found,
    ...
    let insert = (() => {
        var _ref5 = _asyncToGenerator(function* (_ref6) {
            ...
            const result = yield db.insert(collection, _extends({ _id }, systemInfo));
